...
`src/main.cpp`. L'énoncé dit qu'une fonction `main` qui rend zéro suffit ; on a ajouté deux lignes affichées, pour voir aussi le programme tourner.

```cpp
// MaSalle — exercice 1 : nos premiers pas avec Jenga.
...
int main()
{
    std::printf("MaSalle - nos premiers pas avec Jenga\n");
...
Autrement dit, un fichier de projet minimal ne veut pas dire une construction sans réglages : il veut dire que **c'est Jenga qui décide à ma place**. Tout ce que nous n'écrivons pas, il le choisit, et c'est la sortie de construction qui me dit ce qu'il a choisi.
...
MaSalle - nos premiers pas avec Jenga
