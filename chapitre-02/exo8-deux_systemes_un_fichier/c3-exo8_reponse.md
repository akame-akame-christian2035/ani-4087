...
Le programme appelle réellement une fonction de chaque bibliothèque, pour que les liens déclarés soient effectivement sollicités, et il affiche quel filtre il a reçu :
...
`jenga run App` n'a pas eu besoin d'option ici : sans `--platform`, la commande prend la première cible déclarée dans `targetoses`, c'est-à-dire `WINDOWS`, qui est aussi le système de ma machine.
...
`jenga run App` échoue. D'après le code de Jenga consulté, `run` sans `--platform` prend la première cible déclarée dans `targetoses` (`WINDOWS`), alors que `build` choisit le système hôte quand il figure dans cette liste :
...
Le filtre `system:Linux` s'applique donc, `EXO8_LINUX` arrive au programme, et `pthread` et `libm` sont trouvés à l'édition de liens. Le fichier n'a pas eu besoin d'être modifié ; seule la commande d'exécution demande `--platform linux`. D'après le même code, déclarer Linux en premier dans `targetoses` changerait aussi ce comportement de `run` ; je ne l'ai pas essayé.
...
Ces références servent à interpréter le comportement de `run` et de `build` ; les sorties ci-dessus proviennent des exécutions sous Windows et sous Linux.

- [Commande Run : plateforme choisie en l'absence de `--platform`](https://github.com/RihenUniverse/Jenga/blob/7eb93ec423199ef75a043f9249e5f0ece3dfa209/Jenga/Commands/Run.py)
- [Commande Build : choix du système hôte quand il figure dans `targetoses`](https://github.com/RihenUniverse/Jenga/blob/7eb93ec423199ef75a043f9249e5f0ece3dfa209/Jenga/Commands/Build.py)
