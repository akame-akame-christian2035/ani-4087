...
Cette référence confirme, dans le code de Jenga, le mécanisme exact du piège `libc++_shared.so` et l'effet de `androidstl("c++_static")`.

- [`Core/Builders/Android.py` : gestion de `androidstl`, et commentaire décrivant explicitement l'erreur `dlopen failed: library "libc++_shared.so" not found`](https://github.com/RihenUniverse/Jenga/blob/main/Jenga/Core/Builders/Android.py)
