...
Nous avons exécuté `jenga info` dans PowerShell, sur notre ordinateur Windows, depuis
...
## 4. Ce que cette sortie nous apprend que le fichier ne disait pas explicitement

### Les outils réellement détectés

Notre fichier de projet indique C++ et C++17, mais ne nomme pas de compilateur.
...
`Platforms: Windows` est affiché même si nous n'avons pas écrit `platforms(...)`.
Les champs `Target OSes` et `Target Architectures` sont vides parce que nous n'avons
...
notre ordinateur. Les principales informations nouvelles sont les chemins absolus,
